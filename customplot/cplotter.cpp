...
void CPlotter::setPlotOptions()
{
    ...
    //connect (plot, &QCustomPlot::beforeReplot, this, &CPlotter::setRange);

    if (XTickCount) plot->xAxis->ticker()->setTickCount(XTickCount);
    if (qFuzzyCompare(XTickOrigin, 0)) plot->xAxis->ticker()->setTickOrigin(XTickOrigin);
    if (YTickCount) plot->yAxis->ticker()->setTickCount(YTickCount);
    if (qFuzzyCompare(YTickOrigin, 0)) plot->yAxis->ticker()->setTickOrigin(YTickOrigin);
