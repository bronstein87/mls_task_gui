...
void CXYPlotter::addPoint (const double& key, const double& value, PLOT_TYPE type, qint32 seriesPos)
{
    if (!plotIsSetted())
        return;
...
QCPItemTracer* CXYPlotter::setTracer(int numberGraph, double pointX, qint32 colorIndex, bool selectable, QCPItemTracer::TracerStyle style)
{
        const auto& colors = GoodColors::Instance();
        QColor c = colors.getColor(colorIndex);
        return setTracer(numberGraph, pointX, selectable, style, c);
}

QCPItemTracer* CXYPlotter::setTracer(int numberGraph, double pointX, bool selectable, QCPItemTracer::TracerStyle style, QColor color)
{
    QCPItemTracer* tr = new QCPItemTracer(plot);
    tr->setSelectable(selectable);
    tr->setSize(10);
    tr->setStyle(style);
    tr->setBrush(QBrush(color.isValid()? color : QColor(255, 0, 0)));
